<template>
	<div class="diy_details_box diy_div_refund_information">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "refund_information",
					source_field: "refund_information_id",
					source_id: this.obj["refund_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/refund_information/set?refund_information_id=' + _this.obj["refund_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){

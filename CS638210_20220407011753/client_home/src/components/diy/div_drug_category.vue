<template>
	<div class="diy_details_box diy_div_drug_category">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "drug_category",
					source_field: "drug_category_id",
					source_id: this.obj["drug_category_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/drug_category/set?drug_category_id=' + _this.obj["drug_category_id"], {
								praise_len
							}, (res) => {
								if(res.result){

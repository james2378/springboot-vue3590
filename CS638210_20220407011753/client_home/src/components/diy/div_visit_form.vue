<template>
	<div class="diy_details_box diy_div_visit_form">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "visit_form",
					source_field: "visit_form_id",
					source_id: this.obj["visit_form_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/visit_form/set?visit_form_id=' + _this.obj["visit_form_id"], {
								praise_len
							}, (res) => {
								if(res.result){

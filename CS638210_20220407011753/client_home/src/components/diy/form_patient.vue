<template>
	<div>
		<b-form-group v-if="$check_register_field('add','full_name','/patient/view')" id="input-group-0" label="姓名" label-for="input-0">
			<b-form-input id="input-0" v-model="form['full_name']" type="text" placeholder="姓名" trim></b-form-input>
		</b-form-group>
		<b-form-group v-if="$check_register_field('add','gender','/patient/view')" id="input-group-1" label="性别" label-for="input-1">
			<b-form-select id="input-1" v-model="form['gender']" :options="getDropList(list_gender)"></b-form-select>
		</b-form-group>
		<b-form-group v-if="$check_register_field('add','age','/patient/view')" id="input-group-2" label="年龄" label-for="input-2">

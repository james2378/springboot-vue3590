		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/doctor_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_doctor_information = !show_list_doctor_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_doctor_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--医生信息推荐列表组件 -->
		                <list_doctor_information v-if="show_list_doctor_information" :list="list_doctor_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_doctor_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_doctor_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_doctor_information" :key="i" @click="to_details('doctor_information',o,'doctor_information_id')">
		                            <td v-for="(oj,n) in fields_doctor_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/drug_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_drug_information = !show_list_drug_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_drug_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--药品信息推荐列表组件 -->
		                <list_drug_information v-if="show_list_drug_information" :list="list_drug_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_drug_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_drug_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_drug_information" :key="i" @click="to_details('drug_information',o,'drug_information_id')">
		                            <td v-for="(oj,n) in fields_drug_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_doctor_information: true,
				list_doctor_information: [],
				vm_arr_doctor_information: [
				...
				show_list_drug_information: true,
				list_drug_information: [],
				vm_arr_drug_information: [
					"drug_information_id",
					"drug_number",
					"drug_name",
					"drug_cover",
		...
		computed: {
		    list_table_doctor_information() {
		        var list = this.list_doctor_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_doctor_information.map((o) => {
		                // 第二个中括号是对象的属性

		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','patient') || $check_field('add','patient') || $check_field('set','patient')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="患者" prop="patient">
						<el-select v-if="user_group === '管理员' || (form['hospitalization_notice_id'] && $check_field('set','patient')) || (!form['hospitalization_notice_id'] && $check_field('add','patient'))" id="patient" v-model="form['patient']" :disabled="disabledObj['patient_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form['hospitalization_notice_id'] && $check_field('set','full_name')) || (!form['hospitalization_notice_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','prepayment_amount') || $check_field('add','prepayment_amount') || $check_field('set','prepayment_amount')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="预缴金额" prop="prepayment_amount">
					<el-input-number id="prepayment_amount" v-model.number="form['prepayment_amount']"
						v-if="user_group === '管理员' || (form['hospitalization_notice_id'] && $check_field('set','prepayment_amount')) || (!form['hospitalization_notice_id'] && $check_field('add','prepayment_amount'))"></el-input-number>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','doctor_number') || $check_field('add','doctor_number') || $check_field('set','doctor_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="医生编号" prop="doctor_number">
						<el-select v-if="user_group === '管理员' || (form['hospitalization_notice_id'] && $check_field('set','doctor_number')) || (!form['hospitalization_notice_id'] && $check_field('add','doctor_number'))" id="doctor_number" v-model="form['doctor_number']" :disabled="disabledObj['doctor_number_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','diagnostic_results') || $check_field('add','diagnostic_results') || $check_field('set','diagnostic_results')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="诊断结果" prop="diagnostic_results">
					<el-input type="textarea" id="diagnostic_results" v-model="form['diagnostic_results']" placeholder="请输入诊断结果"
						v-if="user_group === '管理员' || (form['hospitalization_notice_id'] && $check_field('set','diagnostic_results')) || (!form['hospitalization_notice_id'] && $check_field('add','diagnostic_results'))" :disabled="disabledObj['diagnostic_results_isDisabled']"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/hospitalization_notice/add?",
				url_set: "~/api/hospitalization_notice/set?",
				url_get_obj: "~/api/hospitalization_notice/get_obj?",
				url_upload: "~/api/hospitalization_notice/upload?",

				query: {
					"hospitalization_notice_id": 0,
				},

				form: {
					"patient": 0, // 患者
					"full_name":'', // 姓名
					"prepayment_amount":0, // 预缴金额
					"doctor_number": 0, // 医生编号
					"diagnostic_results":'', // 诊断结果
					"hospitalization_notice_id": 0, // ID
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/hospitalization_notice/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/hospitalization_notice/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/hospitalization_notice/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/hospitalization_notice/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/hospitalization_notice/view','get');

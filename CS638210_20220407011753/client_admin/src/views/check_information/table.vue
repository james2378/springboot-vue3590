<template>
	<el-main class="bg">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/check_information/table','add') || $check_action('/check_information/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/check_information/table','del') || $check_action('/check_information/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="inspection_time" label="检查时间"
				v-if="user_group == '管理员' || $check_field('get','inspection_time')" min-width="200">
                <template slot-scope="scope">
                	{{ $toTime(scope.row["inspection_time"],"yyyy-MM-dd") }}
                </template>
			</el-table-column>
			<el-table-column prop="inspection_items" label="检查项目"
				v-if="user_group == '管理员' || $check_field('get','inspection_items')" min-width="200">
			</el-table-column>
			<el-table-column prop="check_amount" label="检查金额"
				v-if="user_group == '管理员' || $check_field('get','check_amount')" min-width="200">
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/check_information/table','set') || $check_action('/check_information/view','set') || $check_action('/check_information/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/check_information/table','set') || $check_action('/check_information/view','set') || $check_action('/check_information/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/check_information/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/check_information/get_list?like=0",
				url_del: "~/api/check_information/del?",
		...
		methods: {
			...
			openPayModal(v){
				this.pay_obj.id = v.check_information_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/check_information/set?check_information_id="+this.pay_obj.id);

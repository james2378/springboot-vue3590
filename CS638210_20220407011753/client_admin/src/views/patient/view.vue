			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form_sub['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form_sub['patient_id'] && $check_field('set','full_name')) || (!form_sub['patient_id'] && $check_field('add','full_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['patient_id'] && $check_field('set','gender')) || (!form_sub['patient_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','age') || $check_field('add','age') || $check_field('set','age')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="年龄" prop="age">
					<el-input id="age" v-model="form_sub['age']" placeholder="请输入年龄"
							  v-if="user_group === '管理员' || (form_sub['patient_id'] && $check_field('set','age')) || (!form_sub['patient_id'] && $check_field('add','age'))"></el-input>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["patient_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?patient_id=" + form_sub["patient_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/patient/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/patient/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/patient/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/patient/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/patient/view','get');

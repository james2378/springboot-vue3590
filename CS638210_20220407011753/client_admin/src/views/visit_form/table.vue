<template>
	<el-main class="bg">
			...
			<el-row>
				<el-col :xs="24" :sm="24" :lg="8">
					<el-form-item label="医生姓名">
						<el-input v-model="query.name_of_doctor"></el-input>
					</el-form-item>
				</el-col>
				<el-col :xs="24" :sm="24" :lg="8">
				...
				<el-col :xs="24" :sm="24" :lg="8">
					<el-form-item label="就诊时间">
						<el-input v-model="query.visit_time"></el-input>
					</el-form-item>
				</el-col>
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/visit_form/table','add') || $check_action('/visit_form/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/visit_form/table','del') || $check_action('/visit_form/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="doctor_number" label="医生编号"
				v-if="user_group == '管理员' || $check_field('get','doctor_number')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_doctor_number(scope.row['doctor_number']) }}
				</template>
			</el-table-column>
			...
			<el-table-column prop="patient" label="患者"
				v-if="user_group == '管理员' || $check_field('get','patient')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_patient(scope.row['patient']) }}
				</template>
			</el-table-column>
			...
			<el-table-column prop="visit_time" label="就诊时间"
				v-if="user_group == '管理员' || $check_field('get','visit_time')" min-width="200">
                <template slot-scope="scope">
                	{{ $toTime(scope.row["visit_time"],"yyyy-MM-dd") }}
                </template>
			</el-table-column>
			<el-table-column prop="pharmaceutical_products" label="药品金额品"
				v-if="user_group == '管理员' || $check_field('get','pharmaceutical_products')" min-width="200">
			</el-table-column>
			<el-table-column prop="diagnostic_results" label="诊断结果"
				v-if="user_group == '管理员' || $check_field('get','diagnostic_results')" min-width="200">
			</el-table-column>
			...
			<el-table-column label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/visit_form/table','set') || $check_action('/visit_form/view','set') || $check_action('/visit_form/view','get') || $check_action('/住院/table','add') || $check_action('/住院/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/visit_form/table','set') || $check_action('/visit_form/view','set') || $check_action('/visit_form/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<!--跨表按钮-->
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/hospitalization_notice/view')" v-if="user_group == '管理员' || $check_action('/hospitalization_notice/table','add') || $check_action('/hospitalization_notice/view','add')">
						<span>住院</span>
					</el-button>
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && (user_group == '管理员' || $check_pay('/visit_form/table'))">
						<span>支付</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/visit_form/get_list?like=0",
				url_del: "~/api/visit_form/del?",
		...
		methods: {
			...
			openPayModal(v){
				this.pay_obj.id = v.visit_form_id
				this.payModalVisible = true
			},
			submitPay(){
				let _this = this;
				let url = this.$toUrl(this.query, "~/api/visit_form/set?visit_form_id="+this.pay_obj.id);
